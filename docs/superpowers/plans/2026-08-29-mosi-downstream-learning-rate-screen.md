...
- [ ] 核验继承 Control 配置为 Slot、Regression、all-rates、100 epochs、hidden 200、batch 32、`lr=1e-3`。
- [ ] GPU 0/1/2 分别运行 `--lr 0.0003`、`--lr 0.0005`、`--lr 0.002`；其余参数逐项相同。
- [ ] 每个任务必须产生 100 条 history、metrics 和 8 个 prediction NPZ。
...
- [ ] 从三个新 history 与继承 Control history 读取 `best_validation_mean_weighted_f1`。
- [ ] 在读取 test 汇总前锁定 validation 最优 LR。
- [ ] 若没有新 LR 超过 Control validation，记录 FAIL 并停止。

### 任务 3：候选扩展或关闭

- [ ] 若筛选 PASS，只运行胜出 LR 的 seeds 67--70，Control 继续继承。
- [ ] 若筛选 FAIL，不新增任务。
- [ ] 对正式候选独立重算 NPZ W-F1、核验 mask SHA，并记录逐 rate/seed delta。
