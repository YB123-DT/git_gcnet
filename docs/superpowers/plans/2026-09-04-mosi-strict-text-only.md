...
- [ ] 先写测试，要求改变 Audio/Visual 不改变输出、padding 不影响有效位置、输出形状为 `[L,B,1]` 且 backward 有限。
- [ ] 运行测试并确认因模块不存在而失败。
- [ ] 实现 LayerNorm--Linear--GELU、packed one-layer BiGRU 和 regression head。
- [ ] 运行模型测试并确认通过。
...
- [ ] 先写测试，锁定 validation W-F1 checkpoint selection、五 seeds、唯一 text feature 和无多模态参数。
- [ ] 运行测试并确认预期失败。
- [ ] 复用 official loader/evaluate helpers，实现 100-epoch训练、轻量 JSON/NPZ产物与并发 runner。
- [ ] 运行全部新测试、编译和 diff 检查。
...
- [ ] 同步到 biggpu，使用 GPU 0--3（排除 GPU4）运行 seeds 66--70。
- [ ] 核验每份 history 100 epochs、validation 选优、test 非坍塌与五种子均值。
- [ ] 只同步轻量结果，不提交 checkpoint。
- [ ] 写结论、提交并推送 `feature/missing-m3-target-ple`。
