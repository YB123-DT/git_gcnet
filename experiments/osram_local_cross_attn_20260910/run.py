"""Approved five-seed local-cross-attn run; inherit Flat and select each rate independently."""
import argparse
...
ROOT = Path('/data2/yb/remote_experiments/osram_local_cross_attn_20260910')
...
def configuration(seed):
    ...
    cfg = TrainConfig(**dict(old, osram_readout_fusion='local-cross-attn',
    ...
    assert delta == {'osram_readout_fusion': ['flat', 'local-cross-attn'],
    ...
    provenance = dict(status='training', started_utc=datetime.now(timezone.utc).isoformat(),
                      ...
                      source_sha256={n: sha(REPO / n) for n in
                                     ('gcnet_missing_m3/osram.py', 'gcnet_missing_m3/model.py',
                                      ...
                                      'experiments/osram_local_cross_attn_20260910/run.py')})
    write_json(output / 'PROVENANCE.json', provenance)
    try:
        torch.set_num_threads(6)
        print(f'TRAIN seed={seed} local-cross-attn cyclic eta=.6 epochs=100 per-rate-test-oracle', flush=True)
