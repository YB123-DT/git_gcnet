"""Summarize PAM-E metrics, patterns, paired comparisons, and mechanism stats."""
...
def read_pam_e(root: Path):
    ...
    for seed in SEEDS:
        ...
        for rate in RATES:
            ...
            rows.append({
                'group': 'PAM-E', 'seed': seed, 'rate': rate,
...
def main():
    ...
    pam_t_summary, controls_summary = load_locked_summaries(repo_root)
    ...
    payload = {
        'experiment': 'PAM-E: Explicit Cross-Modal Episodic Text Memory on CMU-MOSI',
        ...
        'comparison_per_rate_mean_percent': {
            'PAM-E': {rate: task[rate]['mean'] for rate in RATES},
        ...
        'pattern_comparison_percent': {
            'PAM-E': {pattern: patterns[pattern] for pattern in PATTERNS},
            'PAM-T': baseline_pam_t_pattern,
            'no-JEPA': pam_t_summary['pattern']['comparison_previous']['no-JEPA'],
            'reg-only': pam_t_summary['pattern']['comparison_previous']['reg-only'],
            'reg+NCE': pam_t_summary['pattern']['comparison_previous']['reg+NCE'],
        },
        'paired': {
            'PAM-E - PAM-T': paired_delta(rows, baseline_pam_t_seed),
            **{
                f'PAM-E - {name}': paired_delta(rows, controls_per_seed[name])
                for name in ('no-JEPA', 'reg-only', 'reg+NCE')
            },
        },
