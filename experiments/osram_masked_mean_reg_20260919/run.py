"""Run a simple masked-mean regression predictor on MOSI."""
...
ROOT = Path("/data2/yb/remote_experiments/osram_masked_mean_reg_20260919")
...
def configuration(seed: int):
    ...
    cfg = replace(
        ...
        simple_regression_predictor=True,
...
def train(seed: int) -> None:
    ...
    if output.exists():
        ...
        if state.exists() and json.loads(state.read_text()).get("status") == "complete":
            print(f"SKIP complete masked-mean-reg seed={seed}", flush=True)
            return
    ...
    provenance = {
        ...
        "label": LABEL,
        "experiment": "MOSI cfg84 simple masked-mean regression predictor",
        "seed": seed,
        ...
        "configuration_delta": {
            ...
            "teacher_mode": "ema",
            "simple_regression_predictor": True,
            "predictor_input": "observed_set_masked_mean_node",
            "predictor": "shared_regression_trunk_plus_target_embedding",
            "osram_output_dim": 1600,
            "osram_key_dim": 64,
            "osram_value_dim": 64,
            "osram_num_heads": 8,
        },
        "removed_modules": ["ContextualM3Predictor", "DualGateTopKMMoE", "InfoNCE"],
        "config": asdict(cfg),
        "reference_sha256": {
    ...
    try:
        ...
        print(
            f"TRAIN masked-mean-reg cfg84 seed={seed} GPU={os.environ.get('CUDA_VISIBLE_DEVICES')}",
            flush=True,
        )
    ...
    write_json(output / "PROVENANCE.json", provenance)
    print(f"COMPLETE masked-mean-reg seed={seed}", flush=True)
...
def launch() -> None:
    ...
    queue = {
        ...
        "label": LABEL,
        "training_objective": "joint-reg-only",
        "predictor": "one masked-mean regression predictor; no MMoE",
        "tasks": [],
    }
