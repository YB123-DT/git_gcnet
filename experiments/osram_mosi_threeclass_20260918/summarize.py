"""Summarize the MOSI three-class-training/nonzero-binary-test diagnostic."""
...
ROOT = Path("/data2/yb/remote_experiments/osram_mosi_threeclass_20260918")
            ...
            rows.append(
                {
                    ...
                    "three_class_wf1": current["weighted_f1"],
                    "no_jepa_regression_accuracy": baseline["accuracy"],
                    "three_class_accuracy": current["accuracy"],
...
def pattern_rows():
    rows = []
    variants = ("no-JEPA-regression", "Three-class")
    stores = {
...
def grouped(rows, rates, value_key="three_class_wf1"):
    pairs = []
    for seed in SEEDS:
        current = [row[value_key] for row in rows if row["seed"] == seed and row["rate"] in rates]
    ...
    return {
        ...
        "three_class_mean": float(np.mean([pair[1] for pair in pairs])),
...
def main():
    ...
    per_rate = []
    for rate in RATES:
        current = [row["three_class_wf1"] for row in rows if row["rate"] == rate]
        baseline = [row["no_jepa_regression_wf1"] for row in rows if row["rate"] == rate]
        current_acc = [row["three_class_accuracy"] for row in rows if row["rate"] == rate]
    ...
    text_groups = {
        group: {
            ...
            for variant in ("no-JEPA-regression", "Three-class")
        }
    ...
    summary = {
        ...
        "configuration_delta": {"mosi_task_mode": ["regression", "three-class"]},
        "metric_protocol": "three-class CE training; test removes original continuous label 0 and compares class-0/class-2 logits",
        "rate": per_rate,
    ...
    for row in per_rate:
        lines.append(
            f"| {row['rate']} | {row['no_jepa_regression_mean']*100:.2f}±{row['no_jepa_regression_std']*100:.2f} | {row['three_class_mean']*100:.2f}±{row['three_class_std']*100:.2f} | {row['delta']*100:+.2f} | {row['no_jepa_regression_accuracy_mean']*100:.2f} | {row['three_class_accuracy_mean']*100:.2f} | {row['accuracy_delta']*100:+.2f} |"
        )
    lines += [
        "",
        f"8-rate mean: regression {summary['overall']['no_jepa_mean']*100:.2f}%, three-class {summary['overall']['three_class_mean']*100:.2f}% ({summary['overall']['delta']*100:+.2f} pp).",
        f"High-missing (.5/.6/.7): regression {summary['high_missing']['no_jepa_mean']*100:.2f}%, three-class {summary['high_missing']['three_class_mean']*100:.2f}% ({summary['high_missing']['delta']*100:+.2f} pp).",
        "",
    ...
    for group, values in text_groups.items():
        for aggregation in ("pattern_macro", "sample_pooled"):
            ...
            c1 = values["Three-class"][aggregation]
            lines.append(f"| {group} | {aggregation} | {c0*100:.2f} | {c1*100:.2f} | {(c1-c0)*100:+.2f} |")
    lines += ["", "## Seven-pattern sample-pooled W-F1", "", "| pattern | regression | three-class | delta |", "|---|---:|---:|---:|"]
    for name in PATTERN_ORDER:
        c0 = summary["patterns"][name]["no-JEPA-regression"]["sample_pooled"]
        c1 = summary["patterns"][name]["Three-class"]["sample_pooled"]
