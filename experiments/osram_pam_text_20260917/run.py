"""Train PAM-T on MOSI with the locked causal OSRAM protocol."""
...
ROOT = Path('/data2/yb/remote_experiments/osram_pam_text_20260917')
...
def configuration(seed):
    ...
    if seed not in SEEDS:
        raise ValueError(f'Unsupported PAM-T seed: {seed}')
...
def train(seed):
    ...
    source_sha = {name: sha(REPO / name) for name in (
        'gcnet_missing_m3/osram.py',
        'gcnet_missing_m3/model.py',
        'gcnet_missing_m3/pam.py',
        'gcnet_missing_m3/b2.py',
        'gcnet_missing_m3/train_gcnet.py',
        'experiments/osram_pam_text_20260917/run.py',
    )}
    provenance = {
        'status': 'training',
    ...
    try:
        ...
        if metrics['selection_protocol'] != 'per-rate-test-oracle':
            raise ValueError('PAM-T did not use per-rate Test-oracle selection')
        reference_metrics = json.loads((source / 'metrics.json').read_text())
        if mask_hashes(metrics) != mask_hashes(reference_metrics):
            raise ValueError('PAM-T test masks differ from the inherited protocol')
    ...
    write_json(output / 'PROVENANCE.json', provenance)
    print(f'COMPLETE PAM-T MOSI seed={seed}', flush=True)


def launch(gpus=(2, 2, 3, 3, 6)):
    import os
    import subprocess
    ...
    for seed in SEEDS:
        ...
        if (ROOT / 'mosi' / f'seed_{seed}').exists():
            raise FileExistsError(f'PAM-T seed {seed} output already exists')
    ROOT.mkdir(parents=True, exist_ok=True)
    manifest = ROOT / 'QUEUE.json'
    if manifest.exists():
        raise FileExistsError(f'PAM-T queue already exists: {manifest}')
    ...
    for seed, gpu in zip(SEEDS, gpus):
        ...
        manifest.write_text(json.dumps(state, indent=2) + '\n')
        print(f'START PAM-T seed={seed} GPU={gpu} PID={child.pid}', flush=True)
