...
This experiment changed GCNet's graph mechanism while inheriting the 40 existing IEMOCAPSix fold-5 Original archives. Original was never retrained. The chronology is preserved: all four candidates first failed the preregistered six-task gate; a later 15-pair RTDR extension passed its bounded criterion; the 40-pair RTDR audit produced `stable_positive=false`; finally, all four arms were compared on a uniform three-rate, five-seed grid. See [RESULTS](results/RESULTS.en.md), [ANALYSIS](results/ANALYSIS.en.md), and the [uniform task evidence](results/uniform_three_rate/RESULTS.en.md).
...
The full audit predefined `stable_positive` as an overall positive macro delta, at least 6/8 positive rate means, at least 3/5 positive seed macros, and finite, non-collapsed runs throughout. Its observed value was `false`: overall delta `-0.002810103`, 3/8 positive rate means, and 3/5 positive seed macros. It is not a retroactive advancement PASS.

## Uniform three-rate evidence

The common `{0.0,0.5,0.7}` × seeds `{66,67,68,69,70}` grid has 15 pairs per arm. It required 27 new trainings—nine each for GenAgg, Soft Medoid, and SSMA—while reusing RTDR's 15 cells. The formal invocation used GPUs 1–7. Three GPU-4 attempts exited `-9`, were moved to diagnostics, and the same task identities completed on GPUs 1–3.

GenAgg: `-0.204847963`, 0/3 positive rates, 0/5 positive seeds, with collapse (`uniform_stable=false`). Soft Medoid: `+0.004706753`, 2/3 rates, 4/5 seeds, non-collapsed, with rate `0.7` at `-0.002089281` (`false`). SSMA: `-0.001153174`, 1/3 rates, 2/5 seeds (`false`). RTDR: `+0.008510981`, 3/3 rates, 3/5 seeds (`true`), but its full-rate `stable_positive=false` conclusion remains. The final unique candidate archive count is 85 = 15 + 15 + 15 + 40; Original training count is zero.

The 40 read-only Original NPZ files under `/data2/yb/paper/experiments/cp_lecc_iemocap6_20260824/protocol_recovery_v1_biggpu/formal/original` were joined by rate, seed, fold, and mask SHA256. Commit `d515386f3207105c8207c34eca3f9743d2b80e4f` implemented the fail-closed legacy-aware validator. Its bounded historical defaults are `branch_fusion=addition`, `pre_graph_context=bilstm`, `post_graph_context=bilstm`, `second_graph_aggregation=add`, and `relation_track_routing=early`. The final unique candidate set contains 85 archives; Original training count remained zero.
