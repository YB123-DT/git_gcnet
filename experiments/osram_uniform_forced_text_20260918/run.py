"""Train no-JEPA causal OSRAM with continuous rates and forced Text omissions."""
...
ROOT = REMOTE / "osram_uniform_forced_text_20260918"
...
def configuration(seed: int):
    ...
    cfg = TrainConfig(
        **dict(
            ...
            uniform_forced_text_probability=0.25,
            checkpoint_selection="test-oracle-per-rate",
        )
    )
    ...
    expected = {"train_rate_mode": ["cyclic", "uniform-forced-text"]}
    if delta != expected:
...
def train(seed: int) -> None:
    ...
    provenance = {
        ...
        "mask_protocol": {
            "train_rate_mode": "uniform-forced-text",
            "rate_distribution": "per-valid-utterance Uniform(0,1)",
            "forced_text_probability": 0.25,
            "empty_row_repair": "retain one modality; preserve forced Text omission when possible",
            "test_protocol": "inherited official fixed rate masks",
        },
        ...
        "source_sha256": {
            ...
            for name in (
                ...
                "experiments/osram_uniform_forced_text_20260918/run.py",
            )
        },
    }
    ...
    try:
        ...
        print(
            f"TRAIN seed={seed} no-JEPA uniform-forced-text pT=.25 eta=.6 epochs={cfg.epochs}",
            flush=True,
        )
