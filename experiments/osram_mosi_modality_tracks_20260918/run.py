"""Train the no-JEPA MOSI control with modality-local emotion tracks."""
...
ROOT = REMOTE / "osram_mosi_modality_tracks_20260918"
...
def configuration(seed: int):
    ...
    cfg = replace(TrainConfig(**old), osram_readout_fusion="modality-tracks")
    before = asdict(TrainConfig(**old))
    after = asdict(cfg)
    delta = {key: [before[key], after[key]] for key in before if before[key] != after[key]}
    expected = {"osram_readout_fusion": ["flat", "modality-tracks"]}
    if delta != expected:
...
def train(seed: int) -> None:
    ...
    provenance = {
        ...
        "model_change": "emotion-only modality tracks; OSRAM memory/query/read/write unchanged",
        ...
        "source_sha256": {
            ...
            for name in (
                ...
                "experiments/osram_mosi_modality_tracks_20260918/run.py",
            )
        },
    }
    ...
    try:
        ...
        print(
            f"TRAIN seed={seed} MOSI modality-tracks no-JEPA cyclic eta=.6 epochs={cfg.epochs}",
            flush=True,
        )
