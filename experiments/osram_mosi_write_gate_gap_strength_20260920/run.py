"""Run the post-ablation MOSI write-gate / Gap-strength controls."""
...
ROOT = REMOTE / "osram_mosi_write_gate_gap_strength_20260920"
REFERENCE_ROOT = REMOTE / "osram_no_aux_cfg84_20260919"
PREVIOUS_ROOT = REMOTE / "osram_mosi_memory_gap_ablation_20260920"
FEATURES = Path(
...
def configuration(variant: str, seed: int):
    ...
    settings = {
        **old,
        ...
        "gap_residual_strength": 0.5 if "r05" in variant else 1.0,
        "beta_mode": "external-head" if variant.startswith("external-beta") else "embedded",
    }
    cfg = TrainConfig(**settings)
    if cfg.osram_output_dim != 1600 or cfg.osram_key_dim != 64 or cfg.osram_value_dim != 64:
        raise ValueError("reference is not cfg84")
    if cfg.osram_num_heads != 8 or cfg.osram_write_step != 0.6:
        raise ValueError("reference is not causal eta=.6")
    return cfg, source
...
def train(variant: str, seed: int) -> None:
    ...
    provenance = {
        ...
        "label": LABEL,
        "experiment": "MOSI cfg84 external beta / Gap residual strength",
        "variant": variant,
        "seed": seed,
        ...
        "configuration_delta": {
            ...
            "gap_residual_strength": cfg.gap_residual_strength,
        },
        "config": asdict(cfg),
        ...
        "source_sha256": {name: sha(REPO / name) for name in (
            ...
            "experiments/osram_mosi_write_gate_gap_strength_20260920/run.py",
        )},
    }
    ...
    try:
        ...
        print(f"TRAIN {variant} seed={seed} GPU={os.environ.get('CUDA_VISIBLE_DEVICES')}", flush=True)
        ...
        if canonical_mask_hashes(output) != canonical_mask_hashes(source):
            raise ValueError("canonical masks differ from cfg84 reference")
...
def launch() -> None:
    ...
    queue = {"status": "running", "started_utc": datetime.now(timezone.utc).isoformat(),
             "label": LABEL, "selection_protocol": "per-rate-test-oracle", "tasks": []}
    write_json(ROOT / "QUEUE.json", queue)
    pending = list(tasks)
    running: dict[int, tuple[subprocess.Popen, object, dict]] = {}
    while pending or running:
        occupied = Counter(row["gpu"] for _, (_, _, row) in running.items())
        free_gpus = [gpu for gpu in GPU_IDS for _ in range(max(0, WORKERS_PER_GPU - occupied[gpu]))]
