"""Train PAM-A: explicit episodic Text memory with direct address supervision."""
...
ROOT = Path('/data2/yb/remote_experiments/osram_pam_address_text_20260918')
SEEDS = (66, 67, 68, 69, 70)
# GPU 4 is known to be unreliable on the current biggpu host.
_DEFAULT_GPUS = (6, 5, 1, 2, 3)
...
def configuration(seed):
    ...
    if seed not in SEEDS:
        raise ValueError(f'Unsupported PAM-A seed: {seed}')
...
def _source_files():
    return (
        ...
        'gcnet_missing_m3/pam_address.py',
        'gcnet_missing_m3/b2.py',
        'gcnet_missing_m3/train_gcnet.py',
        'experiments/osram_pam_address_text_20260918/run.py',
    )
...
def train(seed):
    ...
    try:
        ...
        if metrics['selection_protocol'] != 'per-rate-test-oracle':
            raise ValueError('PAM-A did not use per-rate Test-oracle selection')
        reference_metrics = json.loads((source / 'metrics.json').read_text())
        if mask_hashes(metrics) != mask_hashes(reference_metrics):
            raise ValueError('PAM-A test masks differ from the inherited protocol')
    ...
    write_json(output / 'PROVENANCE.json', provenance)
    print(f'COMPLETE PAM-A MOSI seed={seed}', flush=True)


def _gpus():
    raw = os.environ.get('PAM_A_GPUS', '')
    if raw.strip():
        gpus = tuple(int(value.strip()) for value in raw.split(',') if value.strip())
    else:
        gpus = _DEFAULT_GPUS
    if len(gpus) != len(SEEDS):
        raise ValueError('PAM_A_GPUS must contain one GPU index per seed')
    return gpus
...
def launch():
    ...
    for seed in SEEDS:
        ...
        if (ROOT / 'mosi' / f'seed_{seed}').exists():
            raise FileExistsError(f'PAM-A seed {seed} output already exists')
    ROOT.mkdir(parents=True, exist_ok=True)
    manifest = ROOT / 'QUEUE.json'
    if manifest.exists():
        raise FileExistsError(f'PAM-A queue already exists: {manifest}')
    ...
    for seed, gpu in zip(SEEDS, gpus):
        ...
        manifest.write_text(json.dumps(state, indent=2) + '\n')
        print(f'START PAM-A seed={seed} GPU={gpu} PID={child.pid}', flush=True)
